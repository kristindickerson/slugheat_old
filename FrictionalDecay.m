...
if LowestK < 0
    close(loading)
    uialert(figure_Main, ['Error during iteration. Some thermistors may not' ...
        ' have penetrated. ' 'Must reload penetration file, then remove ' ...
        'bad sensors from data and process again. '],'ERROR!', 'Icon', ...
        'warning')
    pause;
    clc;
    clf reset;
...
try
   ...
   uialert(figure_Main, ['Error: One or more sensors do not have any ' ...
    ...
    ['1. Check heat pulse power. Heat pulse power might be too low. ' ...
    'If this is the case, you must raise power or ignore heat pulse ' ...
    'reduction.'],newline, '2. For sensitivity analysis, check minimum ' ...
    'thermal conductivity values for each sensor. Your minimum may be too ' ...
    'low.'], 'DATA OUTSIDE TAU RANGE', 'Icon','error')
    HPTooLow = 1;
    return
end
clear X Y
...
if any(all(isnan(DataTemp)))
    ...
    uialert(figure_Main, ['Error: One or more sensors do not have any ' ...
        ...
        'reduction.',newline, '2. For sensitivity analysis, check minimum ' ...
        'thermal conductivity values for each sensor. Your minimum may be too ' ...
        'low.' ,newline,newline 'If you wish to use the current heat pulse power, ' ...
        'you must remove sensor(s): ' num2str(whichsens)], 'DATA OUTSIDE TAU RANGE', ...
        'Icon','error')
    HPTooLow = 1;
    return
else
