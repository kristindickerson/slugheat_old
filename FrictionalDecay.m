...
try
    ...
    close(loading)
    uialert(figure_Main, ['Error during iteration. Some thermistors may not have penetrated. ' ...
        'Must reload penetration file, then remove bad sensors from data and process again. '],'ERROR!', 'Icon','warning')
    pause;
    clc;
    clf reset;
    close all hidden;
...
uialert(figure_Main, ['Error: One or more sensors do not have any measurements in the required TAU range. This often occurs when thermal conductivity values are too low.', newline, newline,...
    '1. Check heat pulse power. Heat pulse power might be too low. If this is the case, you must raise power or ignore heat pulse reduction.',newline, ...
    '2. Check minimum thermal conductivity values for each sensor. Your minimum may be too low.'], 'DATA OUTSIDE TAU RANGE', 'Icon','error')
HPTooLow = 1;
return
else
    HPTooLow = 0;
